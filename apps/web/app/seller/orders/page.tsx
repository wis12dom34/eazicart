...
export default function SellerOrdersPage() {
  ...
  if (auth.loading || (auth.isAuthenticated && profile.loading)) {
    return <SellerOrdersShell><LoadingState label="Loading seller orders…" /></SellerOrdersShell>;
  }

  if (!auth.user) {
    return <SellerOrdersShell><SignInState message="Sign in to manage seller orders." /></SellerOrdersShell>;
  }
    ...
    return (
      <SellerOrdersShell>
        <section className={styles.setupPrompt}>
          ...
          <p>Your seller profile is required before order activity can be shown.</p>
    ...
    <SellerOrdersShell>
      <section className={styles.hero}>
        <div>
          ...
          <h1>Orders</h1>
          <p>Only products sold by {profile.data.data.displayName} appear in these order views.</p>
        </div>
      </section>
      ...
      <div className={styles.filters} role="group" aria-label="Filter seller orders">
        {filters.map((value) => (
          <button
            key={value}
            type="button"
      ...
      {!orders.length ? (
        ...
        <div className={styles.filteredEmpty}>No {filterLabels[filter].toLowerCase()} seller orders.</div>
      ) : (
        <section className={styles.orderList} aria-label="Seller orders">
          {visible.map((order) => {
            ...
            return (
              <Link className={styles.orderCard} href={`/seller/orders/${order.id}`} key={order.id}>
                ...
                <div className={styles.cardBody}>
                  <div className={styles.thumb}>
                    {image ? <img src={image.url} alt={image.altText || first?.productName || ""} /> : <Icon name="bag" size={24} />}
                  </div>
                  <div className={styles.cardMeta}>
                    <strong>{money(order.subtotal)}</strong>
                    <span>
                      Your subtotal · {units} {units === 1 ? "unit" : "units"} · {order.items.length} {order.items.length === 1 ? "product" : "products"}
                    </span>
                  </div>
      ...
      <aside className={styles.note}>
        ...
        <p>Only your line items and subtotal are shown. Order status is read-only until EaziCart supports seller-specific fulfillment state.</p>
      </aside>
    </SellerOrdersShell>
  );
}

function SellerOrdersShell({ children }: { children: React.ReactNode }) {
  return (
    <main className={`app-shell ${styles.page}`}>
      <header className={styles.topbar}>
        <Link className={styles.back} href="/seller/dashboard" aria-label="Back to seller dashboard">
